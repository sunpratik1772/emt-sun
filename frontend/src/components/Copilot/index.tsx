...
import { Trash2 as TrashIcon, Edit2, Check, Search, Sparkles } from 'lucide-react'
...
import SherpaActivityChip from './SherpaActivityChip'
...
export default function Copilot() {
  ...
  const activityMode = useWorkflowStore((s) => s.copilotActivityMode)
  ...
  const thinkingStep = thinkingSteps.find(
    (s) => s.kind === 'thinking' || s.subagentType === 'thinking',
  )
  const thinkingMonologue = (thinkingStep?.detail || thinkingStep?.outcome || '').trim()

  useEffect(() => {
    if (!isLoading) return
    if (!thinkingMonologue) {
      setMonologueDone(true)
      return
    }
    setMonologueDone(false)
  }, [isLoading, thinkingMonologue, thinkingStep?.detail])
  const deferReplyForThinking =
    isLoading && Boolean(thinkingMonologue) && !monologueDone
  const showStreamReply =
    Boolean(streamText) &&
    !copilotPendingClarification?.planApproval &&
    !deferReplyForThinking
  ...
  useEffect(() => {
    if (!isLoading) return
    messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' })
  }, [isLoading, thinkingSteps.length, streamText])
  ...
  async function send(msg: string) {
    ...
    if (copilotPendingClarification) {
      toast.warning('Answer the question in the panel below (Continue), or press Skip.')
      return
    }
  ...
  async function onClarificationSubmit(
    answers: import('./sherpaClarificationTypes').SherpaClarificationAnswer[],
  ) {
    await resolveSherpaClarification(answers, criticIter)
  }
  ...
  return (
    <div
      ...
      ref={rootRef}
      className="agent panel-glass flex flex-col relative shrink-0 sherpa-side-panel"
      style={{
        width: copilotWidth,
    ...
    >
      ...
      <div className="agent__head px-4 py-2.5 shrink-0" style={{ borderBottom: '1px solid var(--border-soft)' }}>
        <div className="flex items-center gap-2 w-full">
          <div
            className="agent__avatar flex items-center justify-center shrink-0"
            aria-hidden
          ...
          <span
            className="agent__title display truncate"
            style={{
              fontSize: 13.5,
              fontWeight: 530,
              color: 'var(--text-0)',
          ...
          {/* History, New chat, and Close buttons container */}
          <div className="agent__tools">
            ...
            <button
              ...
              className="agent__tool agent__tool--new lift"
              style={{
                width: 26, height: 26, borderRadius: 5,
                cursor: 'pointer',
              }}
            >
              <ArcIcon icon={Plus} size={14} strokeWidth={2.4} />
            </button>
            <button
        ...
        <div className="agent__msgs">
          ...
          {copilotMessages.length === 0 && (
            <div className="space-y-3">
              ...
              >
                <div className="shrink-0 flex items-center justify-center text-[var(--text-2)]" style={{ width: 16, height: 16 }}>
                  <Sparkles size={13} />
                </div>
                <div
                  style={{
          ...
          {(isLoading || thinkingSteps.length > 0 || streamText || workflowCreated) && (
            <div className="mb-3 flex flex-col gap-2 pl-0.5">
              {(isLoading || copilotPendingClarification) && (
                <SherpaActivityChip mode={activityMode} live={isLoading} />
              )}
      ...
      {copilotPendingClarification ? (
        <SherpaClarificationPanel
          ...
          onSubmit={(answers) => void onClarificationSubmit(answers)}
